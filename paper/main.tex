...
verified through $k=24$). Finally we prove that all interaction in a
...
\begin{proposition}[machine-verified]\label{prop:dccritical}
$DC(k,d)$ with $d$ even is cycle-critical for all tested pairs:
$(k,d)\in\{(8,4),(12,4),(12,6),(16,8),(20,4),(20,10),(22,6),
(22,10),(24,8),(24,12)\}$. With $d$ odd it is freely solvable with
solvable spanning trees in all tested cases.
\end{proposition}

\begin{conjecture}[Double crown dichotomy]\label{conj:dichotomy}
For even $k\ge8$ and $2\le d\le k/2$: $DC(k,d)$ is cycle-critical
iff $d$ is even.
\end{conjecture}

Theorem~\ref{thm:dcs} settles the solvability half of the even case
for the antipodal family; what remains open is that every spanning
tree is unsolvable for all $k$, a double-cluster caterpillar
necessity statement of the same nature as the crown strip. The
solvable holes carry unexpected structure:
...
rank $c$.
\item Prove the criticality half of the double crown dichotomy: for
even $d$, every spanning tree of $DC(k,d)$ is unsolvable.
...
The computations, searches, and drafting were carried out with
extensive assistance from an AI system (Claude, Anthropic). All
machine-verified claims were checked by exhaustive exact search, and
the two model errors found during the work are documented in the
repository's proof notes.
