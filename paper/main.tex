...
last is a $C_8$ with two pendant pairs. This is no accident. If $G$
is solvable and not cycle-critical it has a solvable spanning tree,
so $R=0$; a positive rescue forces $G$ critical or unsolvable. And a
triangle is $K_3$: its third edge lets the loaded corner be refilled
across the clique, worth exactly two pegs when a large cluster is
being cleared, which a spanning path cannot reproduce. Longer cycles
give a smaller boost, which is why $2$ is the maximum. A general
proof would bound the peg loss incurred when one deletes a cycle edge
from an optimal play and replays; the loss is at most two in every
critical instance we tested ($n\le 9$), with the triangle-plus-cluster
family as the worst case.
