...
one inequality is clean and worth recording first.
...
This bounds $R$ by skips but does not bound it absolutely --- and, it
turns out, nothing does. Let $T(q)$ be the triangle with $q$ pendants
...
apparent bound on $n\le 12$ was an artefact of range. Two corrections
follow. First, $T(q)$ for $q\ge 3$ is a second infinite family of
cycle-critical graphs (every tree strands $q-1\ge 2$), triangle-based
rather than even-cycle-based. Second, the strand-count of a
cycle-critical graph is unbounded: the two tiers seen up to $n=14$
...
is $\Theta(q)$, half the maximum pendant load, which suggests a bound
in terms of the largest cluster relative to the cycle.
